...
-- Stability   :  stable
--
-- Modified by : David L Gibbs -- changed the sparse matrix to an IntMap of tuples.
--
...
-- first op -- operates only on the first member of a given tuple
fo :: (a -> a -> a) -> a -> (a,a) -> (a,a)
fo f s (x,y) = ((f s x), y)

-- tuple plus -- add only the first member of the tuple
tppl :: Num a => (a,a) -> (a,a) -> (a,a)
tppl (x,y) (w,z) = (x+w, y) -- pheromones don't matter here --
...
lookupSV k (SV v) = IM.findWithDefault (0,0) k v

-- | Look-up a value in a sparse-matrix.
lookupSM :: Num a => (Int, Int) -> SM a -> (a,a)
lookupSM (i, j) (SM (_, m)) = maybe (0,0) (j `lookupSV`) (i `IM.lookup` m)

-- | Multiply a sparse-vector by a scalar.
sMulSV :: Num a => a -> SV a -> SV a
sMulSV s (SV v) = SV (IM.map (fo (*) s) v)
...
addSV (SV v1) (SV v2) = SV (IM.unionWith tppl v1 v2)

-- | Subtract two sparse vectors.
subSV :: Num a => SV a -> SV a -> SV a
subSV v1 (SV v2) = addSV v1 (SV (IM.map (fo (*) (-1)) v2))

-- | Dot product of two sparse vectors.
dotSV :: Num a => SV a -> SV a -> (a,a)
dotSV (SV v1) (SV v2) = IM.foldl' tppl (0,0) $ IM.intersectionWith tpmu v1 v2
...
normSV :: RealFloat a => SV a -> (a,a)
normSV (SV v) = (\(x,y) -> (sqrt x,y)) . IM.foldl' (\(s,t) (e,f) -> (s+e*e,f)) (0,0) $ v
...
solveCG g a@(SM (n, _)) b = cg a b x0
  where rs = (\(a,b) -> zip a b) $ splitAt n $ take (2*n) $ (randomRs (0.0, 1.0) g) -- was randomRs (0,1)
        x0 = SV $ IM.fromList [p | p@(_, j) <- zip [0..] rs, j /= (0,0)]
...
cg a b x0 = cgIter (1000000 :: Int) (norm r0) r0 r0 x0
 where r0 = b `subSV` (a `mulSMV` x0)
       ...
       cgIter i eps p r x
        ...
        where ap    = a `mulSMV` p                  -- SV
              alpha = eps / (fst (ap `dotSV` p))    -- Double
              x'    = x `addSV` (alpha `sMulSV` p)  -- SV
              r'    = r `subSV` (alpha `sMulSV` ap) -- SV
              eps'  = norm r'
              p'    = r' `addSV` ((eps' / eps) `sMulSV` p)
       norm (SV v) = fst $ IM.foldl' (\(s,t) (e,f) -> (s+e*e,f)) (0.0,0.0) v
...
{-# SPECIALISE INLINE lookupSV :: Int        -> SV Float  -> (Float, Float )                        #-}
{-# SPECIALISE INLINE lookupSV :: Int        -> SV Double -> (Double,Double)                       #-}
{-# SPECIALISE INLINE lookupSM :: (Int, Int) -> SM Float  -> (Float, Float )                        #-}
{-# SPECIALISE INLINE lookupSM :: (Int, Int) -> SM Double -> (Double,Double)                       #-}
...
{-# SPECIALISE INLINE dotSV    :: SV Float  -> SV Float  -> (Float,Float)                         #-}
{-# SPECIALISE INLINE dotSV    :: SV Double -> SV Double -> (Double,Double)                        #-}
{-# SPECIALISE INLINE mulSMV   :: SM Float  -> SV Float  -> SV Float                      #-}
{-# SPECIALISE INLINE mulSMV   :: SM Double -> SV Double -> SV Double                     #-}
{-# SPECIALISE INLINE normSV   :: SV Float  -> (Float,Float)                                      #-}
{-# SPECIALISE INLINE normSV   :: SV Double -> (Double,Double)                                     #-}
