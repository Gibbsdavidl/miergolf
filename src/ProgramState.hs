...
data State = 
  State {    runs :: Int,  -- the number of restarts
             ...
             bestEver :: (Double, Digraph), -- best over all
             bestRest :: (Double, Digraph), -- best since the last restart
             bestIter :: (Double, Digraph), -- best this ant-iteration.
...
fillState1 gen xs y z = State {runs = read (xs !! 1) :: Int,
                          ...
                          bestEver = (0.0, []) :: (Double, Digraph),
                          bestRest = (0.0, []) :: (Double, Digraph),
                          bestIter = (0.0, []) :: (Double, Digraph),
                          g = gen,
                          randomSeed = 0,
                          config = y,
                          graphfile = z,
...
showState :: State -> ScoreData -> String
showState s scoredata = 
  (show (config s)) ++ "\t" ++ 
  (show (graphfile s)) ++ "\t" ++ 
  (show (length (nub (listNodes (fst scoredata))))) ++ "\t" ++
  (show (length (fst scoredata))) ++ "\t" ++
  (show (randomSeed s)) ++ "\t" ++
  (show (k s)) ++ "\t" ++ 
  (show (runs s)) ++ "\t" ++ 
  (show (ants s)) ++ "\t" ++ 
  ...
  (show (rx s)) ++ "\t" ++ 
  (show (ptoKratio s)) ++ "\t" ++
  (showSolution (bestEver s)) ++ "\n"

  
showSolution :: (Double, Digraph) -> String
showSolution (solnScore, soln) = (show solnScore) ++ "\t" ++ (prettyPrintGraph soln ("\n"))
